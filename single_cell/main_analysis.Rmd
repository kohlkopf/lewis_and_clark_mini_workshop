...
Plots aren't just the end product of analysis--plots like the following can assis the analyst in determing filtering thresholds and other necessary parameters.
...
After removing unwanted cells from the dataset, the next step is to normalize the data. By default, we employ a global-scaling normalization method “LogNormalize” that normalizes the gene expression measurements for each cell by the total expression, multiplies this by a scale factor (10,000 by default), and log-transforms the result. This is necessary to be able to perform cell-cell contrasts. It addresses the issue of certain cells having higher overall expression--gene expression become relative within the cell and can be compared to gene expression in other cells.  
...
Gene level z-scores are alos calculated at this step. The the mean total expression for a gene is zero-centered, and the standard deviation is stretched to 1. Z-scores allow us to make gene-gene comparisons even if one gene is universally more highly expressed.
